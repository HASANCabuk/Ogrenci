...
namespace OBS
{
    class DKTROGR:YLISANSOGR
    {       
        ArrayList ylunvrs = new ArrayList();
        ArrayList ylblm = new ArrayList();
        internal override void OgrTut(OGRENCI ogr)
        {
            if (lNo.Count == 0)
            {
                ...
                ylunvrs.Add(ogr.OgrMznYLsnsU);
                ylblm.Add(ogr.OgrMznYLsnsBlm);
            }
            else
            {
       ...
       internal override void OgrGoster()
        {
            ...
            else
            {
                ...
                for (int i = 0; i < lNo.Count; i++)
                {
                    ...
                    Console.WriteLine("\t YÜKSEK LİSANS BİLGİLERİ");
                    Console.WriteLine("\t\t{0}; {0}", ylunvrs[i], ylblm[i]);
